...
select date_of_joining,
dayname(date_of_joining) as day_of_week
from employeeattendance;

-- Q4. Find the total number of days each employee has worked in the company
--     (from date_of_joining till today).
select emp_name,date_of_joining,work_date,
datediff(curdate(),date_of_joining) as total_number_of_day
from employeeattendance;
...
round((datediff(curdate(),date_of_joining))/365,0) as completed_year
from employeeattendance;
...
timestampdiff(hour,clock_in,clock_out) as total_hour
from employeeattendance;

-- Q7. Extract the hour at which each employee clocked in.
select emp_name,clock_in,
hour(clock_in) as hour_clock_in_time
from employeeattendance;
